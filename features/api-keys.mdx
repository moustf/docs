...
```
┌────────────────────────────┬──────────────────────┬─────────────────────┬─────────────────────┐
│ Name                       │ Value                │ Created At          │ Expires At          │
├────────────────────────────┼──────────────────────┼─────────────────────┼─────────────────────┤
│ Production API Key         │ sk_live_****...****  │ 2024-01-15 10:30    │ 2025-12-31 23:59    │
│ Development Key            │ sk_live_****...****  │ 2024-01-10 09:15    │ Never               │
│ CI/CD Pipeline             │ sk_live_****...****  │ 2024-01-05 14:22    │ 2024-07-05 14:22    │
└────────────────────────────┴──────────────────────┴─────────────────────┴─────────────────────┘
```
...
```bash
curl -X PATCH https://api.tryhamsa.com/v1/api-keys?projectId=YOUR_PROJECT_ID \
...
```bash
curl -X GET https://api.tryhamsa.com/v1/calls \
...
```bash
curl -X POST https://api.tryhamsa.com/v1/voice-agents/phone-number/call \
  -H "Authorization: Bearer sk_live_your_key_here_xxxxx" \
  -H "Content-Type: application/json" \
  -d '{
    "projectId": "your-project-id",
...
const rotateApiKey = async () => {
  ...
  await slack.sendMessage({
    channel: '#engineering',
    text: `🔑 API key rotated. Old key will be revoked in 48 hours.`,
  });
...
curl -v https://api.tryhamsa.com/v1/calls \
  -H "Authorization: Bearer sk_live_your_key_here_xxxxx"
...
# Common mistakes:
❌ -H "Authorization: sk_live_your_key_here_xxxxx"  # Missing "Bearer"
❌ -H "API-Key: sk_live_your_key_here_xxxxx"        # Wrong header name
❌ -H "Bearer: sk_live_your_key_here_xxxxx"         # Wrong format
```
...
const testKey = async (apiKey) => {
  try {
    const response = await fetch('https://api.tryhamsa.com/v1/calls', {
      headers: {
